package com.yilian.luckypurchase.widget;
...
public class NumberAddSubView extends LinearLayout implements View.OnClickListener {
    ...
    public NumberAddSubView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (attrs != null) {
            final TypedArray a = context.obtainStyledAttributes(attrs,
                    R.styleable.Lucky_NumberAddSubView, defStyleAttr, 0);

            int layout = a.getResourceId(R.styleable.Lucky_NumberAddSubView_lucky_lefenlayout, R.layout.lucky_widet_num_add_sub);
            setLayout(layout);
            initView();

            int val = a.getInt(R.styleable.Lucky_NumberAddSubView_lucky_value, 0);
            setValue(val);

            int maxVal = a.getInt(R.styleable.Lucky_NumberAddSubView_lucky_maxValue, 0);
            if (maxVal != 0)
                setMaxValue(maxVal);

            int minVal = a.getInt(R.styleable.Lucky_NumberAddSubView_lucky_minValue, 0);
            setMinValue(minVal);

            Drawable etBackground = a.getDrawable(R.styleable.Lucky_NumberAddSubView_lucky_editBackground);
            if (etBackground != null)
                setEditTextBackground(etBackground);

            Drawable buttonAddBackground = a.getDrawable(R.styleable.Lucky_NumberAddSubView_lucky_buttonAddBackgroud);
            if (buttonAddBackground != null)
                setButtonAddBackgroud(buttonAddBackground);

            Drawable buttonSubBackground = a.getDrawable(R.styleable.Lucky_NumberAddSubView_lucky_buttonSubBackgroud);
    ...
    private void initView() {
        ...
        mEtxtNum.setInputType(InputType.TYPE_CLASS_NUMBER);
