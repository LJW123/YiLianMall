...
public class GridPasswordView extends LinearLayout implements PasswordView {
    ...
    private void initAttrs(Context context, AttributeSet attrs, int defStyleAttr) {
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.library_module_gridPasswordView, defStyleAttr, 0);

        textColor = ta.getColorStateList(R.styleable.library_module_gridPasswordView_library_module_textColor);
        if (textColor == null)
            textColor = ColorStateList.valueOf(getResources().getColor(R.color.library_module_holo_gray_light));
        int textSize = ta.getDimensionPixelSize(R.styleable.library_module_gridPasswordView_library_module_textSize, -1);
        if (textSize != -1) {
            this.textSize = com.yilian.mylibrary.widget.Util.px2sp(context, textSize);
        }

        lineWidth = (int) ta.getDimension(R.styleable.library_module_gridPasswordView_library_module_lineWidth, com.yilian.mylibrary.widget.Util.dp2px(getContext(), 1));
        lineColor = ta.getColor(R.styleable.library_module_gridPasswordView_library_module_lineColor, DEFAULT_LINECOLOR);
        gridColor = ta.getColor(R.styleable.library_module_gridPasswordView_library_module_gridColor, getResources().getColor(R.color.library_module_color_white));
        lineDrawable = ta.getDrawable(R.styleable.library_module_gridPasswordView_library_module_lineColor);
        if (lineDrawable == null)
            lineDrawable = new ColorDrawable(lineColor);
        outerLineDrawable = generateBackgroundDrawable();

        passwordLength = ta.getInt(R.styleable.library_module_gridPasswordView_library_module_passwordLength, DEFAULT_PASSWORDLENGTH);
        passwordTransformation = ta.getString(R.styleable.library_module_gridPasswordView_library_module_passwordTransformation);
        if (TextUtils.isEmpty(passwordTransformation))
            passwordTransformation = DEFAULT_TRANSFORMATION;

        passwordType = ta.getInt(R.styleable.library_module_gridPasswordView_library_module_passwordType, 0);
    ...
    private void inflaterViews(Context context) {
        ...
        inflater.inflate(R.layout.library_module_gridpasswordview, this);
        ...
        while (index < passwordLength) {
            View dividerView = inflater.inflate(R.layout.library_module_gridpasswordview_divider, null);
            ...
            TextView textView = (TextView) inflater.inflate(R.layout.library_module_gridpasswordview_textview, null);
