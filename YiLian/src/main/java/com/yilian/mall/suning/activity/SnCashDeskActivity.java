...
import com.yilian.mall.suning.presenter.SnPayPresenterImpl;
 ...
 * @author  京东收银台
 */
public class SnCashDeskActivity extends BaseAppCompatActivity implements View.OnClickListener, JDOrderPayPresenter.View, IUserMoneyView {

    public static final String TAG = "SnCashDeskActivity";
    ...
    private SnCommitOrderEntity snCommitOrderSuccessEntity;
    /**
     * 用户奖励是否足够支付商品
     */
    ...
    private void initData() {
        snCommitOrderSuccessEntity = (SnCommitOrderEntity) getIntent().getSerializableExtra(TAG);
        getUserMoney();
    }
    ...
    private void pay(int payType) {
        if (jdPayCashPresenter == null) {
            jdPayCashPresenter = new SnPayPresenterImpl(
                    mContext,
                    this,
                    snCommitOrderSuccessEntity.id,
                    String.valueOf(
    ...
    public void paySuccess(HttpResultBean jdPayInfoEntity) {
        ((SnPayPresenterImpl) jdPayCashPresenter).onDestory();
        JumpJdActivityUtils.toJDJDPaySuccessActivity(mContext, jdPayInfoEntity, JDPaySuccessActivity.PaySide.SN);
        finish();
// 更新苏宁订单列表信息
        //EventBus 推送
        ...
        updateModel.jdOrderId = snCommitOrderSuccessEntity.orderId;
        /**
    ...
    public void getUserMoneySuccess(MyBalanceEntity2 myBalanceEntity2) {
        ...
        float orderTotalPrice = com.yilian.mylibrary.NumberFormat.convertToFloat(
                getOrderPrice(snCommitOrderSuccessEntity.amount,
                        snCommitOrderSuccessEntity.freight, snCommitOrderSuccessEntity.daiGouQuanMoney), 0);
        ...
        if (userMoney >= orderTotalPrice) {
            ...
            orderPriceWithoutUserCash = NumberFormat.convertToFloat(
                    new BigDecimal(String.valueOf(orderTotalPrice))
                            .subtract(new BigDecimal(String.valueOf(userMoney)))
                    , 0f);
        }
        setData(snCommitOrderSuccessEntity);
     ...
     * @return
     */
    String getOrderPrice(String goodPrice, String freight, String daiGouQuan) {
        return String.valueOf(new BigDecimal(goodPrice).add(new BigDecimal(freight)).subtract(new BigDecimal(daiGouQuan)));
    }

    private void setData(SnCommitOrderEntity snCommitOrderEntity) {
        tvOrderCashPayMoney.setText(String.format("¥%s", orderPricePartOfUserCash));
        RxUtil.countDown(NumberFormat.convertToInt(snCommitOrderEntity.time, 0))
