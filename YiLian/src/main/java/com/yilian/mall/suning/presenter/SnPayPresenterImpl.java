package com.yilian.mall.suning.presenter;
...
import static com.yilian.mylibrary.pay.ThirdPayForType.PAY_FOR_SN_GOODS;

/**
 * @author Created by  on 2018/5/28.
 */

public class SnPayPresenterImpl implements JDOrderPayPresenter, PasswordFinished {
    /**
     * 支付宝
     */
    ...
    public SnPayPresenterImpl(Context context, View view, String orderIndex, String merchantIncomePayMoney) {
...
//            跳转设置支付密码页面
    ...
    private void payThird(float thirdFee) {
        ...
        Subscription subscription = RetrofitUtils3.getRetrofitService(context)
                .snPreRecharge("suning_pay/suning_recharge_order", String.valueOf(thirdFee),
                        ALI_PAY, orderIndex, PAY_FOR_SN_GOODS,mMerchantIncomePayMoney)
    ...
    public void passwordFinished(String psw) {
        view.startMyDialog(false);
        Subscription subscription = RetrofitUtils3.getRetrofitService(context)
                .snPay("suning_pay/suning_pay_balance", orderIndex, psw,mMerchantIncomePayMoney)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
    ...
    private void getPayResult() {
        Subscription subscription = RetrofitUtils3.getRetrofitService(context)
                .getSnPayInfo("pay_info",orderIndex , ThirdPayForType.PAY_FOR_SN_GOODS)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<SnPayInfoEntity>() {
                    @Override
                    public void onCompleted() {
                    ...
                    @Override
                    public void onNext(SnPayInfoEntity jdPayInfoEntity) {
