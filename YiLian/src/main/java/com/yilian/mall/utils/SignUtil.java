...
public class SignUtil {
    ...
    public static String sign(String method , Map<String, String> headersParams , String pathWithParameter , Map<String, String> queryParams , Map<String, String> formParam) {
        try {
            ...
            byte[] keyBytes = Constants.APP_SECRET.getBytes(Constants.CLOUDAPI_ENCODING);
    ...
    private static String buildStringToSign(String method , Map<String, String> headerParams, String pathWithParameter, Map<String, String> queryParams , Map<String, String> formParams) {
        ...
        if (headerParams.get(Constants.CLOUDAPI_HTTP_HEADER_ACCEPT) != null) {
            sb.append(headerParams.get(Constants.CLOUDAPI_HTTP_HEADER_ACCEPT));
        }
        sb.append(Constants.CLOUDAPI_LF);

        //如果有@"Content-MD5"头，这个头需要参与签名
        if (headerParams.get(Constants.CLOUDAPI_HTTP_HEADER_CONTENT_MD5) != null) {
            sb.append(headerParams.get(Constants.CLOUDAPI_HTTP_HEADER_CONTENT_MD5));
        }
        sb.append(Constants.CLOUDAPI_LF);

        //如果有@"Content-Type"头，这个头需要参与签名
        if (headerParams.get(Constants.CLOUDAPI_HTTP_HEADER_CONTENT_TYPE) != null) {
            sb.append(headerParams.get(Constants.CLOUDAPI_HTTP_HEADER_CONTENT_TYPE));
        }
        sb.append(Constants.CLOUDAPI_LF);

        //签名优先读取HTTP_CA_HEADER_DATE，因为通过浏览器过来的请求不允许自定义Date（会被浏览器认为是篡改攻击）
        if (headerParams.get(Constants.CLOUDAPI_HTTP_HEADER_DATE) != null) {
            sb.append(headerParams.get(Constants.CLOUDAPI_HTTP_HEADER_DATE));
        }
        sb.append(Constants.CLOUDAPI_LF);
    ...
    private static String buildHeaders(Map<String, String> headers) {
        ...
        if (headers != null) {
            ...
            //同时所有加入签名的头的列表，需要用逗号分隔形成一个字符串，加入一个新HTTP头@"X-Ca-Signature-Headers"
            headers.put(Constants.CLOUDAPI_X_CA_SIGNATURE_HEADERS, signHeadersStringBuilder.toString());
        }
