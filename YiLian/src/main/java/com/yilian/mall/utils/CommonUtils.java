...
public class CommonUtils {
    ...
    public static boolean serivceReturnCode(Context context, int code, String msg) {
        switch (code) {
            ...
            case -4:
            case -23:
                PushManager.getInstance().unBindAlias(context, com.yilian.mylibrary.PreferenceUtils.readStrConfig(Constants.USER_ID, context), false);
                PreferenceUtils.writeBoolConfig(Constants.SPKEY_STATE, false, context);
                Intent intent = new Intent(context, LeFenPhoneLoginActivity.class);
    ...
    public static Boolean NetworkRequestReturnCode(Context context, String code) {
        ...
        switch (code) {
            case "1":
                return true;
            case "-1":
                Toast.makeText(context, R.string.sixty_time_one, Toast.LENGTH_SHORT).show();
                return false;
            case "-2":
                Toast.makeText(context, "系统限制", Toast.LENGTH_SHORT).show();
                return false;
    ...
    public static boolean isPhoneNumer(String mobiles) {
        ...
        String telRegex = PHONE_REGEX;//"[1]"代表第1位为数字1，"[358]"代表第二位可以为3、5、8中的一个，"\\d{9}"代表后面是可以是0～9的数字，有9位。
