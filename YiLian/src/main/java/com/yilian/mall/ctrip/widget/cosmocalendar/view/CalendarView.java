...
import com.yilian.mall.ctrip.widget.cosmocalendar.FetchMonthsAsyncTask;
import com.yilian.mall.ctrip.widget.cosmocalendar.adapter.MonthAdapter;
import com.yilian.mall.ctrip.widget.cosmocalendar.listeners.OnMonthChangeListener;
...
import com.yilian.mall.ctrip.widget.cosmocalendar.selection.NoneSelectionManager;
import com.yilian.mall.ctrip.widget.cosmocalendar.selection.OnDaySelectedListener;
import com.yilian.mall.ctrip.widget.cosmocalendar.selection.RangeSelectionManager;
import com.yilian.mall.ctrip.widget.cosmocalendar.selection.SingleSelectionManager;
import com.yilian.mall.ctrip.widget.cosmocalendar.selection.selectionbar.MultipleSelectionBarAdapter;
import com.yilian.mall.ctrip.widget.cosmocalendar.selection.selectionbar.SelectionBarItem;
import com.yilian.mall.ctrip.widget.cosmocalendar.settings.SettingsManager;
import com.yilian.mall.ctrip.widget.cosmocalendar.settings.appearance.AppearanceInterface;
import com.yilian.mall.ctrip.widget.cosmocalendar.settings.date.DateInterface;
import com.yilian.mall.ctrip.widget.cosmocalendar.settings.lists.CalendarListsInterface;
import com.yilian.mall.ctrip.widget.cosmocalendar.settings.lists.DisabledDaysCriteria;
...
import com.yilian.mall.ctrip.widget.cosmocalendar.settings.selection.SelectionInterface;
...
import com.yilian.mall.ctrip.widget.cosmocalendar.utils.WeekDay;
import com.yilian.mall.ctrip.widget.cosmocalendar.utils.snap.GravitySnapHelper;
import com.yilian.mall.ctrip.widget.cosmocalendar.view.customviews.CircleAnimationTextView;
import com.yilian.mall.ctrip.widget.cosmocalendar.view.customviews.SquareTextView;
import com.yilian.mall.ctrip.widget.cosmocalendar.view.delegate.MonthDelegate;
...
//        }
//    };
    private String[] weekend = new String[]{"六", "日"};
    private View rlTopTitles;
    ...
    private void createRecyclerView() {
        ...
        params.addRule(RelativeLayout.BELOW, llTitle.getId());
        rvMonths.setLayoutParams(params);

        rvMonths.setLayoutManager(new GridLayoutManager(getContext(),
                1, settingsManager.getCalendarOrientation(), false));
...
//        flBottomSelectionBar.setLayoutTransition(new LayoutTransition());
        flBottomSelectionBar.setId(R.id.fl_bottom_selection_bar);
    ...
    private void createDaysOfWeekTitle() {
        ...
        if (!isTitleAlreadyAdded) {
            ...
            llDaysOfWeekTitles.setId(R.id.ll_days_of_week_titles);
        ...
        for (String title : CalendarUtils.createWeekDayTitles(settingsManager.getFirstDayOfWeek())) {
            ...
            if (title.equals(weekend[0]) || title.equals(weekend[1])) {
                tvDayTitle.setTextColor(ContextCompat.getColor(getContext(), R.color.color_red));
            } else {
                tvDayTitle.setTextColor(ContextCompat.getColor(getContext(), R.color.color_333));
            }
            llDaysOfWeekTitles.addView(tvDayTitle);
        }

        //adding borders
        llDaysOfWeekTitles.setBackgroundColor(Color.WHITE);

        if (!isTitleAlreadyAdded) {
            llTitle = new LinearLayout(getContext());
            llTitle.setId(R.id.ll__titles);
            llTitle.setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT));
            llTitle.setOrientation(LinearLayout.VERTICAL);

            View titleTop = LayoutInflater.from(getContext()).inflate(R.layout.title_calendar_view, null);
            View tvCancel = titleTop.findViewById(R.id.tv_cancel);
            ...
            llTitle.addView(titleTop);
            llTitle.addView(llDaysOfWeekTitles);
    ...
    private void createMultipleSelectionBarRecycler() {
        ...
        rvMultipleSelectedList.setId(R.id.rv_multiple_selected_list);
    ...
    private void createRangeSelectionLayout() {
        llRangeSelection =
                (LinearLayout) (
                        ...
                        .inflate(R.layout.view_selection_bar_range, null);
...
//            rvMonths.scrollToPosition(lastVisibleMonthPosition);
    ...
    private void handleAttributes(TypedArray typedArray) {
        ...
        int selectedDayBackgroundStartColor
                ...
                typedArray.getColor(R.styleable.CalendarView_selectedDayBackgroundStartColor,
                        ContextCompat.getColor(getContext(), R.color.default_selected_day_background_start_color));
        int selectedDayBackgroundEndColor
                =
                typedArray.getColor(R.styleable.CalendarView_selectedDayBackgroundEndColor,
                        ContextCompat.getColor(getContext(), R.color.default_selected_day_background_end_color));
    ...
    private void changeSnapHelper() {
        ...
        if (snapHelper == null) {
            snapHelper
                    = new GravitySnapHelper(settingsManager.getCalendarOrientation() == LinearLayoutManager.VERTICAL ?
                    Gravity.TOP : Gravity.START, true, this);
    ...
    private void loadAsyncMonths(final boolean future) {
        if (asyncTask != null &&
                (asyncTask.getStatus() == AsyncTask.Status.PENDING || asyncTask.getStatus() == AsyncTask.Status.RUNNING)) {
            return;
        }
        ...
        asyncTask.execute(
                new FetchMonthsAsyncTask.FetchParams(future, month, settingsManager, monthAdapter, SettingsManager.DEFAULT_MONTH_COUNT));
    }
    ...
    private void displaySelectedDaysRange() {
        if (selectionManager instanceof RangeSelectionManager) {
            ...
            if (days != null) {
                Log.i("时间选择区间", "入住时间：" + days.first.toString() + "   离店时间：" + days.first.toString());
    ...
    public void setOnDaysSelectedListener(OnDaysSelected onDaysSelected) {
        mOnDaysSelected = onDaysSelected;
    }
...
//        changeSnapHelper();
    ...
    public void setMaxRange(int maxRange) {
        if (selectionManager instanceof RangeSelectionManager) {
            ((RangeSelectionManager) selectionManager).setMaxRange(maxRange);
        }
    }
