package com.yilian.mall.ctrip.presenter;
...
import static com.yilian.mylibrary.pay.ThirdPayForType.PAY_FOR_CTRIP;

/**
 * @author Created by Zg on 2018/10/26.
 */

public class CtripPayPresenterImpl implements JDOrderPayPresenter, PasswordFinished {
    /**
     * 支付宝
     */
    ...
    public CtripPayPresenterImpl(Context context, View view, String orderIndex, String merchantIncomePayMoney) {
...
//            跳转设置支付密码页面
    ...
    private void payThird(float thirdFee) {
        ...
        Subscription subscription = RetrofitUtils3.getRetrofitService(context)
                .ctripPreRecharge("xc_pay/xc_recharge_order", String.valueOf(thirdFee),
                        ALI_PAY, orderIndex, PAY_FOR_CTRIP,mMerchantIncomePayMoney)
    ...
    public void passwordFinished(String psw) {
        view.startMyDialog(false);
        Subscription subscription = RetrofitUtils3.getRetrofitService(context)
                .ctripPay("xc_pay/xc_pay_balance", orderIndex, psw,mMerchantIncomePayMoney)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
    ...
    private void getPayResult() {
        Subscription subscription = RetrofitUtils3.getRetrofitService(context)
                .getCtripPayInfo("pay_info",orderIndex , PAY_FOR_CTRIP)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<CtripPayInfoEntity>() {
                    @Override
                    public void onCompleted() {
                    ...
                    @Override
                    public void onNext(CtripPayInfoEntity jdPayInfoEntity) {
