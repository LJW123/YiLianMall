...
import com.yilian.networkingmodule.entity.ExchangePayByPasswordSuccessEntity;
...
public class ExchangeOrderPayPresenter implements JDOrderPayPresenter, PasswordFinished {
    /**
     * 支付宝
     */
    ...
    private JDOrderPayPresenter.View view;
    private String orderIndex;
    private PayDialog payDialog;
    private CompositeSubscription compositeSubscription;

    public ExchangeOrderPayPresenter(Context context,
                                     View view, String orderIndex, String merchantIncomePayMoney) {
...
//            跳转设置支付密码页面
    ...
    private void payThird(float thirdFee) {
        ...
        Subscription subscription = new OrderPayModelImpl().createThirdPayOrder(context, PayType.ALI_PAY,
                MoneyUtil.getLeXiangBi(thirdFee), "0", ThirdPayForType.PAY_FOR_EXCHANGE, orderIndex, mMerchantIncomePayMoney)
                ...
                .subscribe(new Observer<JsPayClass>() {
                    ...
                    @Override
                    public void onNext(JsPayClass o) {
                        aliPay(o);
                    }
                });
     ...
     * @param o
     */
    private void aliPay(JsPayClass o) {
    ...
    public void passwordFinished(String psw) {
        ...
        Subscription subscription = RetrofitUtils3.getRetrofitService(context)
                .exchangePayByPassword("agent/pay_agent_balance",
                        orderIndex, psw, mMerchantIncomePayMoney)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<ExchangePayByPasswordSuccessEntity>() {
                    @Override
                    public void onCompleted() {
                        cancelPayDialog();
                        view.stopMyDialog();
                    ...
                    @Override
                    public void onNext(ExchangePayByPasswordSuccessEntity o) {
                        getPayResult();
                    }
                });
        addSubscription(subscription);
    ...
    private void getPayResult() {
        view.paySuccess(new HttpResultBean());
    }
