...
public class  ScrollViewContainer extends RelativeLayout {
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			if (mMoveLen != 0) {
				if (state == AUTO_UP) {
					mMoveLen -= SPEED;
					if (mMoveLen <= -topHeight) {
						mMoveLen = -topHeight;// -mViewHeight;
						state = DONE;
						mCurrentViewIndex = 1;
					}
				...
				if (mSwitchCallback != null) {
					mSwitchCallback.onMove((int) mMoveLen, topView, bottomView);
				}
			}
			requestLayout();
		}
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_UP:
			...
			if (mMoveLen == 0 || mMoveLen == -topHeight)
				break;
			if (Math.abs(mYV) < 500) {
	...
	public void setSwitchCallbak(SwitchCallback callback) {
	...
	public interface SwitchCallback {
		 ...
		 * @param bottomView
		 */
		public abstract void onMove(int moveY, View topView, View bottomView);
	}
}