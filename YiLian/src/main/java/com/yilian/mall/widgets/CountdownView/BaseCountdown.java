package com.yilian.mall.widgets.CountdownView;
...
class BaseCountdown {
    ...
    public void initStyleAttr(Context context, TypedArray ta) {
        ...
        mTimeTextSize = ta.getDimension(R.styleable.CountdownView_timeTextSize, DPXUnitUtil.sp2px(12, mContext));
        ...
        mSuffixTextSize = ta.getDimension(R.styleable.CountdownView_suffixTextSize, DPXUnitUtil.sp2px(12,mContext));
    ...
    private void initSuffixMargin() {
        int defSuffixLRMargin = DPXUnitUtil.dp2px(mContext, DEFAULT_SUFFIX_LR_MARGIN);
    ...
    private float getDayAndHourContentWidth() {
        ...
        if (isShowDay) {
            String tempDay = StringFormat.formatNum(mDay);
        ...
        if (isShowHour) {
            String tempHour = StringFormat.formatNum(mHour);
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (isShowDay) {
            // draw day text
            canvas.drawText(StringFormat.formatNum(mDay), mLeftPaddingSize + mDayTimeTextWidth / 2, mTimeTextBaseline, mTimeTextPaint);
        ...
        if (isShowHour) {
            ...
            canvas.drawText(StringFormat.formatNum(mHour), mHourLeft + curTimeTextWidth / 2, mTimeTextBaseline, mTimeTextPaint);
        ...
        if (isShowMinute) {
            // draw minute text
            canvas.drawText(StringFormat.formatNum(mMinute), mMinuteLeft + mTimeTextWidth / 2 , mTimeTextBaseline, mTimeTextPaint);
        ...
        if (isShowSecond) {
            // draw second text
            canvas.drawText(StringFormat.formatNum(mSecond), mSecondLeft + mTimeTextWidth / 2, mTimeTextBaseline, mTimeTextPaint);
            if (mSuffixSecondTextWidth > 0) {
                // draw second suffix
            ...
            if (isShowMillisecond) {
                ...
                canvas.drawText(StringFormat.formatMillisecond(mMillisecond), mMillisecondLeft + mTimeTextWidth / 2, mTimeTextBaseline, mTimeTextPaint);
    ...
    public void setTimeTextSize(float textSize) {
        if (textSize > 0) {
            mTimeTextSize = DPXUnitUtil.sp2px(textSize,mContext);
            mTimeTextPaint.setTextSize(mTimeTextSize);
        }
    }
    ...
    public void setSuffixTextSize(float textSize) {
        if (textSize > 0) {
            mSuffixTextSize = DPXUnitUtil.sp2px( textSize,mContext);
            mSuffixTextPaint.setTextSize(mSuffixTextSize);
        }
    }
    ...
    public void setSuffixLRMargin(float suffixLRMargin) {
        mSuffixLRMargin = DPXUnitUtil.dp2px(mContext, suffixLRMargin);
    ...
    public boolean setSuffixMargin(Float suffixDayMarginL, Float suffixDayMarginR,
        ...
        if (null != suffixDayMarginL) {
            mSuffixDayLeftMargin = DPXUnitUtil.dp2px(mContext, suffixDayMarginL);
            isRef = true;
        }
        if (null != suffixDayMarginR) {
            mSuffixDayRightMargin = DPXUnitUtil.dp2px(mContext, suffixDayMarginR);
            isRef = true;
        }

        if (null != suffixHourMarginL) {
            mSuffixHourLeftMargin = DPXUnitUtil.dp2px(mContext, suffixHourMarginL);
            isRef = true;
        }
        if (null != suffixHourMarginR) {
            mSuffixHourRightMargin = DPXUnitUtil.dp2px(mContext, suffixHourMarginR);
            isRef = true;
        }

        if (null != suffixMinuteMarginL) {
            mSuffixMinuteLeftMargin = DPXUnitUtil.dp2px(mContext, suffixMinuteMarginL);
            isRef = true;
        }
        if (null != suffixMinuteMarginR) {
            mSuffixMinuteRightMargin = DPXUnitUtil.dp2px(mContext, suffixMinuteMarginR);
            isRef = true;
        }

        if (null != suffixSecondMarginL) {
            mSuffixSecondLeftMargin = DPXUnitUtil.dp2px(mContext, suffixSecondMarginL);
            isRef = true;
        }
        if (null != suffixSecondMarginR) {
            mSuffixSecondRightMargin = DPXUnitUtil.dp2px(mContext, suffixSecondMarginR);
            isRef = true;
        }

        if (null != suffixMillisecondMarginL) {
            mSuffixMillisecondLeftMargin = DPXUnitUtil.dp2px(mContext, suffixMillisecondMarginL);
            isRef = true;
        }
