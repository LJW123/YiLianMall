...
/*
 * Copyright (C) 2011 The Android OpenInitialPayActivityBroadcastReceiver Source Project
 *
...
public class LazyViewPager extends ViewGroup {
    ...
    void populate() {
        ...
        if (mPopulatePending) {
            if (DEBUG)
                Logger.i(TAG, "populate is pending, skipping for now...");
            return;
        }
        ...
        for (int i = 0; i < mItems.size(); i++) {
            ...
            if ((ii.position < startPos || ii.position > endPos)
                    && !ii.scrolling) {
                if (DEBUG)
                    Logger.i(TAG, "removing: " + ii.position + " @ " + i);
                mItems.remove(i);
                i--;
                mAdapter.destroyItem(this, ii.position, ii.object);
                ...
                while (lastPos <= endPos && lastPos < ii.position) {
                    if (DEBUG)
                        Logger.i(TAG, "inserting: " + lastPos + " @ " + i);
        ...
        if (lastPos < endPos) {
            ...
            while (lastPos <= endPos) {
                if (DEBUG)
                    Logger.i(TAG, "appending: " + lastPos);
    ...
    public void computeScroll() {
        if (DEBUG)
            Logger.i(TAG, "computeScroll: finished=" + mScroller.isFinished());
        if (!mScroller.isFinished()) {
            if (mScroller.computeScrollOffset()) {
                if (DEBUG)
                    Logger.i(TAG, "computeScroll: still scrolling");
                int oldX = getScrollX();
                int oldY = getScrollY();
     ...
     * {@link com.yilian.mall.widgets.LazyViewPager.OnPageChangeListener}
