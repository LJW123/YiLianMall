...
import com.yilian.mylibrary.widget.ImeDelBugFixedEditText;
...
public class GridPasswordView extends LinearLayout implements PasswordView {
    ...
    private com.yilian.mylibrary.widget.ImeDelBugFixedEditText inputView;
    ...
    private void initAttrs(Context context, AttributeSet attrs, int defStyleAttr) {
        TypedArray ta = context.obtainStyledAttributes(attrs, com.yilian.mylibrary.R.styleable.library_module_gridPasswordView, defStyleAttr, 0);

        textColor = ta.getColorStateList(com.yilian.mylibrary.R.styleable.library_module_gridPasswordView_library_module_textColor);
        if (textColor == null)
            textColor = ColorStateList.valueOf(getResources().getColor(com.yilian.mylibrary.R.color.library_module_holo_gray_light));
        int textSize = ta.getDimensionPixelSize(com.yilian.mylibrary.R.styleable.library_module_gridPasswordView_library_module_textSize, -1);
        if (textSize != -1) {
            this.textSize = Util.px2sp(context, textSize);
        }

        lineWidth = (int) ta.getDimension(com.yilian.mylibrary.R.styleable.library_module_gridPasswordView_library_module_lineWidth, Util.dp2px(getContext(), 1));

        lineColor = ta.getColor(com.yilian.mylibrary.R.styleable.library_module_gridPasswordView_library_module_lineColor, DEFAULT_LINECOLOR);
        gridColor = ta.getColor(com.yilian.mylibrary.R.styleable.library_module_gridPasswordView_library_module_gridColor, getResources().getColor(com.yilian.mylibrary.R.color.library_module_color_white));
        lineDrawable = ta.getDrawable(com.yilian.mylibrary.R.styleable.library_module_gridPasswordView_library_module_lineColor);
        if (lineDrawable == null)
            lineDrawable = new ColorDrawable(lineColor);
        outerLineDrawable = generateBackgroundDrawable();

        passwordLength = ta.getInt(com.yilian.mylibrary.R.styleable.library_module_gridPasswordView_library_module_passwordLength, DEFAULT_PASSWORDLENGTH);
        passwordTransformation = ta.getString(com.yilian.mylibrary.R.styleable.library_module_gridPasswordView_library_module_passwordTransformation);
        if (TextUtils.isEmpty(passwordTransformation))
            passwordTransformation = DEFAULT_TRANSFORMATION;

        passwordType = ta.getInt(com.yilian.mylibrary.R.styleable.library_module_gridPasswordView_library_module_passwordType, 0);
    ...
    private void inflaterViews(Context context) {
        ...
        inflater.inflate(com.yilian.mylibrary.R.layout.library_module_gridpasswordview, this);
        inputView = (com.yilian.mylibrary.widget.ImeDelBugFixedEditText) findViewById(com.yilian.mylibrary.R.id.inputView);
        ...
        while (index < passwordLength) {
            View dividerView = inflater.inflate(com.yilian.mylibrary.R.layout.library_module_gridpasswordview_divider, null);
            ...
            TextView textView = (TextView) inflater.inflate(com.yilian.mylibrary.R.layout.library_module_gridpasswordview_textview, null);
    ...
    private ImeDelBugFixedEditText.OnDelKeyEventListener onDelKeyEventListener = new com.yilian.mylibrary.widget.ImeDelBugFixedEditText.OnDelKeyEventListener() {
