    ...
    public NumberAddSubView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (attrs != null) {
            final TypedArray a = context.obtainStyledAttributes(attrs,
                    R.styleable.MerchantNumberAddSubView, defStyleAttr, 0);

            int layout = a.getResourceId(R.styleable.MerchantNumberAddSubView_merchant_lefenlayout, R.layout.merchant_widet_num_add_sub_shopping_cart);
            setLayout(layout);
            initView();

            int val = a.getInt(R.styleable.MerchantNumberAddSubView_merchant_value, 0);
            setValue(val);

            int maxVal = a.getInt(R.styleable.MerchantNumberAddSubView_merchant_maxValue, 0);
            if (maxVal != 0)
                setMaxValue(maxVal);

            int minVal = a.getInt(R.styleable.MerchantNumberAddSubView_merchant_minValue, 0);
            setMinValue(minVal);

            Drawable etBackground = a.getDrawable(R.styleable.MerchantNumberAddSubView_merchant_editBackground);
            if (etBackground != null)
                setEditTextBackground(etBackground);

            Drawable buttonAddBackground = a.getDrawable(R.styleable.MerchantNumberAddSubView_merchant_buttonAddBackgroud);
            if (buttonAddBackground != null)
                setButtonAddBackgroud(buttonAddBackground);

            Drawable buttonSubBackground = a.getDrawable(R.styleable.MerchantNumberAddSubView_merchant_buttonSubBackgroud);
